...
@export var Motion : EntityMotionComponent
...
@export var time_to_duck = 0.5

@export_group("Player View")
var offset : float = 0.711 # Current offset from player's origin.
var desired_offset: float = 0.711 # Dessired offset from player's origin.
...
## Updated to be whatever the current position is.
...
func _physics_process(delta) -> void:
	...
	if Root.player_input.noclip_enabled:
		move_noclip(delta)
		return
	
	
	if ducked or ducking:
		offset = desired_offset
	else:
		offset = Util.exp_decay(offset, desired_offset, 8.0, delta)
	
	view_target.transform.origin.y = offset

	# Add the gravity.
	if not is_on_floor():
		velocity.y -= Config.GRAVITY * delta * gravity_multiplier
	...
	try_step()
	move_body()
	#var local_offset = Vector3.RIGHT * Root.player_camera.camera.global_basis * -0.2
	#DebugDraw3D.draw_line(current_position + local_offset, global_position + local_offset, Color.GREEN, 2.5)
	var total_motion = global_position - current_position
	Root.player_sounds.add_motion(total_motion)
...
func try_step() -> bool:
	...
	if collision:
		...
		if normal.y < 0.7:
			result = move_step(normal)
	elif step_was_on_floor and not is_on_floor() and not Input.is_action_pressed("pm_jump"):
		result = move_step_down()
	step_was_on_floor = is_on_floor()
	return result
...
func move_noclip(delta) -> void:
	var multiplier = 1.0
	if Input.is_action_pressed("pm_evade"):
		multiplier = 3.0
...
func duck(duck_on: bool) -> void:
	...
	if duck_on:
		if !ducked and !ducking:
			ducking = true
			duck_timer.start(time_to_duck)
		
		time = max(0, (time_to_duck - duck_timer.time_left))
		
		if ducking:
			if duck_timer.time_left <= time_to_duck * 0.6 or !is_on_floor():
				# Set the collision hull and view offset to the ducking counterpart.
				set_ducked(true)
				desired_offset = Config.DUCK_VIEW_OFFSET
				ducking = false
			else:
				frac = spline_fraction(inverse_lerp(0.0, time_to_duck, time), 2.5)
				desired_offset = ((Config.DUCK_VIEW_OFFSET - crouch_dist ) * frac) + (Config.VIEW_OFFSET * (1-frac))
...
func unduck() -> void:
	...
	if cast_static_trace(self, BBOX_DUCKING, position + Vector3.UP * crouch_dist) != null:
		...
		set_ducked(false)
		ducking = false
		desired_offset = Config.VIEW_OFFSET
...
func cast_static_trace(what: CollisionObject3D, shape: Shape3D, from: Vector3) -> Trace:
	...
	params.set_collision_mask(TraceMask.SOLID_FOR_PLAYER)
...
func move_step(normal: Vector3) -> bool:
	...
	if trace.end_pos[1] > original_pos[1] and trace.normal[1] >= 0.7:
		var difference = global_position - trace.end_pos
		Root.entity_view.offset_camera(difference)
		global_transform.origin = trace.end_pos
		#velocity = velocity.slide(trace.normal)
		var down_check = cast_trace(self, BBOX_STEP, 
		global_position + Vector3.UP * 0.1,
		global_position + Vector3.DOWN * 0.2)
	
		if down_check and down_check.fraction < 1.0:
			global_position.y = down_check.end_pos.y
		return true
...
func move_step_down():
	...
	# Cast down from a point slightly in front of us
	var forward_offset = velocity.normalized() * 0.3
	var start_pos = original_pos + forward_offset + Vector3.UP * 0.1
	var end_pos = start_pos + Vector3.DOWN * (Config.STEP_HEIGHT + 0.2)
	
	var trace = cast_trace(self, BBOX_STEP, start_pos, end_pos)
	
	if trace and trace.fraction < 1.0:
		var diff = original_pos - trace.end_pos
		var height_diff = diff.y
		if height_diff > 0.05 and height_diff <= Config.STEP_HEIGHT:
			var old_pos = global_position
			global_position.y = trace.end_pos.y
			var cam_diff = old_pos - global_position
			Root.entity_view.offset_camera(cam_diff)
			return true
	
	return false

func apply_impulse(added_velocity: Vector3):
	Motion.apply_velocity(added_velocity)
...
func set_entity_state(state_dict):
	ducked = state_dict["ducked"]
	velocity = state_dict["velocity"]
	global_position = state_dict["position"]
